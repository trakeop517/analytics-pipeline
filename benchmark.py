...
async def run_benchmark():
    ...
    await client.delete(DLQ_QUEUE_NAME)
    print(f" Generating: Генерируем {TOTAL_EVENTS} уникальных событий по схеме базы...")
    start_push = time.time()
    pipeline = client.pipeline()
    for i in range(TOTAL_EVENTS):
        fake_event = {"source": "benchmark_test", "external_id": f"evt_{i}_{int(time.time())}", "title": f"Stress Test Event #{i}", "payload": {"metric": "performance", "step": i}, "created_at": datetime.utcnow().isoformat() }
