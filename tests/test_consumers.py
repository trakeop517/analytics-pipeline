...
from app.consumers.worker import save_batch_with_retry, send_to_dlq, worker_loop
...
async def test_save_batch_db_error_triggers_dlq():
    """Тест: При постоянной ошибке БД воркер делает ретраи и сливает батч в DLQ."""
    ...
    with patch("app.consumers.worker.async_session_factory") as mock_factory, \
        ...
        assert mock_redis.lpush.call_args[0][0] == "events:dlq"
...
async def test_worker_loop_parses_iso_date_and_flushes_batch():
    ...
    mock_redis = MagicMock()
    mock_redis.brpop = AsyncMock(side_effect=[("events_queue", raw_event), asyncio.CancelledError()])
    captured_batches = []
